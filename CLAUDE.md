...
- **Command mode** (`internal/tui/commands.go`): Typing `:` pushes a command context onto the context stack. Commands are normal tree items — `version` is a folder with `on`/`off` leaves. Navigation uses the same fzt interaction model (arrows, typing to filter, Enter to drill in). Selecting a leaf executes the command via `onLeafSelect` callback and pops the context. Escape pops back to the previous context.
...
- **WASM API consolidation**: Removed `initSession` (flat mode) and `initTree` alias from the WASM bridge. `fzt.init()` now always creates a tree session — tree mode is the only interaction model for WASM consumers.
- **Build script** (`build/main.go`): `go run ./build` replaces `go build .` for local builds. Injects git describe version via ldflags automatically. `go run ./build wasm` builds the WASM. Running bare `go build .` produces a binary that errors on startup ("version not set") to prevent accidentally forgetting ldflags.
- **Build version at build time**: `version_native.go` (build tag `!js`) runs `git describe --tags --always --dirty` at startup for native builds. WASM builds get version via ldflags in CI or the build script. Eliminates the gap where Claude (or the user) runs `go build .` without ldflags and gets "dev".
- **Shared terminal component** (`web/fzt-terminal.js`): Extracted the ANSI parser, grid renderer, font metrics, key forwarding, click handling, and resize logic into a single shared ES module. Both fzt-showcase and my-homepage import it instead of maintaining their own ~400-line renderers. Shipped as a release asset alongside `fzt.wasm`. Theme config (palette, fonts, cursor class, padding) passed via options object. Includes the BMP PUA wide-char fix that was only in my-homepage's renderer.
